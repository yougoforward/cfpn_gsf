...
class cfpnHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.project4 = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 1, padding=0, dilation=1, bias=False),
        ...
        self.pool4 = nn.Sequential(nn.AdaptiveAvgPool2d(4),
                                   nn.Conv2d(in_channels, inter_channels, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU(),
                                   )
        self.pool8 = nn.Sequential(nn.AdaptiveAvgPool2d(8),
                                   nn.Conv2d(in_channels, inter_channels, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU(),
                                   )
    ...
    def forward(self, c1,c2,c3,c4):
        ...
        p4_1 = F.interpolate(pool2, (h,w), **self._up_kwargs)
        p4_8 = F.interpolate(pool4, (h,w), **self._up_kwargs)
        p3_1 = F.interpolate(pool8, (h,w), **self._up_kwargs)
        p3_8 = F.interpolate(p4, (h,w), **self._up_kwargs)
        p2_8 = F.interpolate(out3, (h,w), **self._up_kwargs)
