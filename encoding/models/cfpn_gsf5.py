...
__all__ = ['cfpn_gsf55', 'get_cfpn_gsf55']


class cfpn_gsf55(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(cfpn_gsf55, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = cfpn_gsf55Head(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class cfpn_gsf55Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(cfpn_gsf55Head, self).__init__()
        ...
        self.nl1 = PAM_Module2(in_dim=inter_channels, key_dim=inter_channels//8,value_dim=inter_channels,out_dim=inter_channels,norm_layer=norm_layer)
        self.nl2 = PAM_Module2(in_dim=inter_channels, key_dim=inter_channels//8,value_dim=inter_channels,out_dim=inter_channels,norm_layer=norm_layer)
        self.nl3 = PAM_Module2(in_dim=inter_channels, key_dim=inter_channels//8,value_dim=inter_channels,out_dim=inter_channels,norm_layer=norm_layer)
        self.nl4 = PAM_Module2(in_dim=inter_channels, key_dim=inter_channels//8,value_dim=inter_channels,out_dim=inter_channels,norm_layer=norm_layer)
        self.nlp_conv = nn.Sequential(nn.Conv2d(3*inter_channels, inter_channels, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU(),
                                   )
    ...
    def forward(self, c1,c2,c3,c4):
        ...
        out3 = self.nl1(out)
        out4 = self.nl1(out)
        out = self.nlp_conv(torch.cat([out1, out2, out3, out4, out], dim=1))
...
def get_cfpn_gsf55(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = cfpn_gsf55(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
...
class PAM_Module2(nn.Module):
    ...
    def __init__(self, in_dim, key_dim, value_dim, out_dim, norm_layer):
        ...
        self.softmax = nn.Softmax(dim=-1)
        self.val_conv = nn.Sequential(nn.Conv2d(in_dim, in_dim//2, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(in_dim//2),
                                   nn.ReLU())
